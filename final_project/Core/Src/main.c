...
typedef struct vibStruct {
	int status;
} vibrate;

typedef size_t uart_ret;
/* USER CODE END PTD */
...
#define VIBRATE_RUN 0
#define VIBRATE_STOP 1
...
/* Private user code ---------------------------------------------------------*/
/* USER CODE BEGIN 0 */
const size_t ubuffSize = 30;
char ubuff[30];

char sudo[10];
char cmd[10];
char msg[10];
int mode = 0, office = 1, game = 1;
int isplay = 0;

int rec = 0;
HAL_StatusTypeDef hal_rec;
